...
## Weather data
KEY="9eeff4fba37af6dfc3bd99a5413e466d"
ID="1816670"
...
## Get data
get_weather_data() {
    weather=`curl -sf "https://api.openweathermap.org/data/2.5/weather?lat={24.9917}&lon={121.2990}&appid=${KEY}"`
    echo ${weather}

    if [ ! -z "$weather" ]; then
        weather_temp=$(echo "$weather" | jq ".main.temp" | awk '{printf "%.0f\n",$1-273.15}')
...
# if [[ ! -f "$cache_weather_degree" ]]; then
#     get_weather_data
# fi
